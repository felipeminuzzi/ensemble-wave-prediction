...
def create_multi_graph(files):
    ...
    for f in files:
        ...
        model   = f.split('/')[-1].split('_')[2][:-4]
        df.set_index('Data', inplace=True)
        df.columns = [f'erro_real_{j}',f'erro_{model}', f'Hs_{model}', f'Hs_real_{j}', f'Noaa_{model}']
        dfs_6.append(df)
        j += 1
    ...
    erros_6['mean'] = erros_6.mean(axis=1)
    ...
    plt.figure(3).text(0.5, 0.04, 'Time', ha='center', va='center')
    plt.figure(3).text(0.06, 0.5, 'Wave height - Hs (m)', ha='center', va='center', rotation='vertical')

    plt.figure(4)
    create_plots_multi(lead,reais[0],predictions[0],xx[0],yy[0], True, True, 'Noaa_cnn-lstm')
    plt.figure(4).text(0.5, 0.04, 'Time', ha='center', va='center')
    plt.figure(4).text(0.06, 0.5, 'Wave height - Hs (m)', ha='center', va='center', rotation='vertical')
...
def create_plots_multi(lead, df_true, df_predict,x,y, flag, antigo, tgt):
    ls      = df_predict.columns.to_list()
    leg_dict = {'erro_real_1':'Error NOAA x Buoy', 'Hs_real_1':'Real buoy data', 'Noaa_cnn-lstm':'NOAA deterministic prediction'}
    lab = leg_dict[tgt]
    plt.title(f'Deep learning prediction of Hs')
    
    if antigo:
        plt.plot(df_true.index, df_true['Hs_real_1'], '-*', label=f'Real buoy data',color='black')
        mapes = mape(df_true['Hs_real_1'], df_true['Noaa_cnn-lstm']).round(2)
    else:
        mapes = 0

    plt.plot(df_true.index, df_true[tgt] , '--', label=f'{lab} - {mapes}', color='blue')
    if flag:
        for col in ls[:-1]:
            if antigo:
...
def create_error_multi(lead, df_true, df_predict,x,y, flag, antigo, tgt):
    ...
    for col in ls[-1:]:
        ...
        plt.plot(df_predict.index, error2 , '-', label=f'Predicted {col}')
    plt.legend()
    plt.xticks(x,y, rotation=30)
